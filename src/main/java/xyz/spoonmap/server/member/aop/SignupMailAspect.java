...
public class SignupMailAspect {
    ...
    @AfterReturning(value = "execution(* xyz.spoonmap.server.member.service.MemberService.signUp(..))", returning = "returnValue")
    public void test(SignupResponse returnValue) {
        MimeMessage message = javaMailSender.createMimeMessage();

        setMail(message, returnValue.id(), returnValue.email());
        javaMailSender.send(message);

        log.info("Success Send Signup Mail to [{}]", returnValue.email());
    }
    ...
    private String setMailContent(String email, Long id) {
        ...
        return springTemplateEngine.process("signup", context);
    }
